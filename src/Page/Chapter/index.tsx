...
export default function Chapter() {
    ...
    const { chapter, loading, error } = useArrChaptersDetails(chapterIdNumber)
    useEffect(() => {
        // Cuộn lên đầu trang mỗi khi component được render hoặc khi chapter thay đổi
        if (user) {
            postHistory(user?.id, chapterIdNumber)
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            if (event.key === 'ArrowLeft' && chapter?.preIndex) {
                history.push(`/novel/${chapter.chapter.novelId}/${chapter.preIndex}`);
            } else if (event.key === 'ArrowRight' && chapter?.nextIndex) {
                history.push(`/novel/${chapter.chapter.novelId}/${chapter.nextIndex}`);
            }
        };
        ...
        <div className={stylePage}>
            ...
            <div className="md:container ">
                <div className="text-sm p-2  ">
                    <span className="border-b-[1px] border-[#555555]">{chapter?.novelTitle} / {chapter?.chapter.title}</span>
                </div>
                <div className="m-5">
                    <Link to={`/novel/${chapter?.chapter.novelId}`}>
                        <div className="flex justify-center my-3 h-6 text-xl font-black text-[#808080]">{chapter?.novelTitle}</div>
                    </Link>
                    <div className="flex text-base text-[#808080] justify-center">{chapter?.chapter.title}</div>
                </div>
                ...
                <div className="bg-[#EEEEEE] flex text-base font-semibold p-1 text-[#008800]">
                    ...
                    {chapter?.preIndex !==null ?
                        <Link to={`../novel/${chapter?.chapter.novelId}/${chapter?.preIndex}`}>
                    ...
                    {chapter?.nextIndex !== null ?
                        <Link to={`../novel/${chapter?.chapter.novelId}/${chapter?.nextIndex}`}>
                ...
                <div className={styleContent}>
                    {loading ?
                        (<Loading />)
                        : chapter ? (<ChapterContent filePath={`${chapter?.chapter.content}`}></ChapterContent>
                        )
                            : (<div>
                                Error: {error}
                                <br />
